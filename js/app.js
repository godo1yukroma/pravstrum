(() => {
    ...
    function loadImage(srcs, isCrossOrigin) {
        if (!("object" === typeof srcs && "Array" === srcs.constructor.name)) srcs = [ srcs ];
        let promises = [];
        srcs.forEach((src => {
    ...
    (async function main() {
        const image = "/img/footer/01.png";
        const c = document.getElementById("webgl");
        const speed = .4;
        const maxStrength = 13;
        const minStrength = 3;
        const maxRadius = c.offsetWidth < 768 ? .08 : .3;
        const minRadius = c.offsetWidth < 768 ? .01 : .05;
        ...
        const webgl = new Kgl({
            ...
            tick: time => {
                const progress = .5 * Math.sin(time * speed) + .5;
                webgl.bindFramebuffer("mask");
                webgl.programs["mask"].draw({
                    progress
                });
                webgl.effects["godray"].draw("mask", "cache", "output", mix(maxStrength, minStrength, progress), [ mix(-.2 * webgl.canvas.width, .3 * webgl.canvas.width, progress), 1.2 * webgl.canvas.height ], mix(maxRadius, minRadius, progress), true);
            }
        });
    })();
    window["FLS"] = true;
